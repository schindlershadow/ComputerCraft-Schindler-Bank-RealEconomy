...
for k,v in pairs(xflippable) do xflippable[v] = k end
for k,v in pairs(xinvertable) do xinvertable[v] = k end
...
local images = {
	logo = {
	  ...
	  },
	  {
	    ...
	    "         fa11aa11a11a111a11a1aaa111ffaaaaaaaa3f  ",
	    "         faaa1aaa1aa1aaaa1aa1aa1aaaaeeeeeeee33af ",
	    "         fa1a1a1a1aa1aaaa1aa1aa1aaaeeeeeee00eeeea",
	    ...
	    "f1aa111111a11aaa11a11aa11111a111a11a aaaaaaaa    ",
	  },
	  {
	    ...
	    "         a1aa11aa1aa1aaa1aa1aaa1aaaaaeeeeeee3aa  ",
	    "         a111a1111aa1aaa1aa1aaa111aeeeeeeeee333a ",
	    "         a1aa11aa1aa1aaa1aa1aaa1aaaeeeeeeeeeeeeaf",
	    "         a111a1aa1aa1aaa1aa111a111aeeeeeeeeeeeeaf",
	    "a1aa1a1aaaa1aa1aaaaaa1a1a1a1a1a1a1aeeeeeeeeeeeeaf",
	    "a11a1a111aa1aaa1a11a1aa1a1a111a11aa333eeeeeeeeaf ",
	    "a1aa1a1aaaa1aaaa11a11aa1a1a1a1a1a1aaaaeeeeaaaaf  ",
	    "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaf ffffffff    ",
	  },
	},
	win = {
		{
	...
	prism = {{"","",""},{"f03f","3333","    "},{"0300","3003"," 33f"}},
	cannon = {{"",""},{"ff","77"},{"77","  ",}},
	boomer = {{""," "},{"ccff"," ffc"},{"  cc"," ccf"}},
	wideshot = {{" ","",""," "," ","",""," "},{"f ","f9","ff"," 9"," 9","f9","f9","9 "},{"9 ","9f","99"," 9"," 9","9f","9f","  "}},
	buster = {{""},{"f4"},{"4f"}}
}
...
act.stage.setType = function(x, y, panelType)
	x, y = round(x), round(y)
	if act.stage.checkExist(x, y) then
		stage.panels[y][x].panelType = panelType
...
act.stage.getType = function(x, y)
	x, y = round(x), round(y)
	if act.stage.checkExist(x, y) then
		return stage.panels[y][x].panelType
	else
		return false
	end
end
	...
	if act.stage.checkExist(x, y) then
		...
		if math.max(0, math.min(maxCrack, stage.panels[y][x].crackedLevel + amount)) ~= stage.panels[y][x].crackedLevel then
			...
			if stage.panels[y][x].crackedLevel == 2 then
				...
				stage.panels[y][x].panelType = "normal"
...
act.stage.setDamage = function(x, y, damage, owner, time, noFlinch, safePlayers, safeObjects, isInvisible)
	x, y = round(x), round(y)
			...
			for k, v in pairs(stage.damage[y][x]) do
				if k ~= (players[pID] or {}).owner and k ~= (objects[oID] or {}).owner and v.damage then
					if not (v.safePlayers[pIDsafeCheck] or v.safeObjects[oIDsafeCheck]) then
						...
						isInvisible = isInvisible and v.isInvisible
					end
				end
			end
		end
	end
	return totalDamage, flinching, isInvisible
end
...
act.player.newPlayer = function(x, y, owner, direction, image)
	local pID = #players + 1
	players[pID] = {
		x = round(x),					-- X and Y positions are relative to grid, not screen
		y = round(y),					-- ditto my man
		...
		cooldown = {					-- All cooldown values are decremented every tick
			...
			struck = 0						-- If above 0, you are registered as having been hit
		},
		control = {
			moveUp = false,
			moveDown = false,
		...
		folder = testMode and premadeFolders["test"] or premadeFolders[math.random(1, 3)]
	}
	return pID
end

act.object.hurtObject = function(oID, damage, flinching, ignoreInvincibility)
	local object = objects[oID]
	if object.cooldown.iframe == 0 or ignoreInvincibility then
		object.health = math.max(0, object.health - damage)
		if object.health == 0 then
			table.remove(objects, oID)
		elseif flinching then
			object.cooldown.iframe = 2
		end
		if object.onHitFunc then
			if not object.onHitFunc(object, damage, flinching) then
				table.remove(objects, oID)
			end
		end
		return true
...
act.player.hurtPlayer = function(pID, damage, flinching, ignoreInvincibility)
	local player = players[pID]
	if player.cooldown.iframe == 0 or ignoreInvincibility then
...
act.object.newObject = function(x, y, owner, direction, objectType)
	...
	objects[oID] = {
		...
		delayedFunc = objectTypes[objectType].delayedFunc or function() return true end,
		onHitFunc = objectTypes[objectType].onHitFunc or function() return true end,
		xvel = 0,
		yvel = 0,
		owner = owner,
		direction = direction,
...
act.player.movePlayer = function(pID, xmove, ymove, doCooldown)
	...
	if (xmove ~= 0 or ymove ~= 0) and act.stage.checkIfWalkable(player.x + xmove, player.y + ymove, pID) then
		repeat
			...
			if act.stage.getType(player.x, player.y) == "ice" then
				render()
				if isHost then
					transmit({
						gameID = gameID,
		...
		until (not act.stage.checkIfWalkable(player.x + xmove, player.y + ymove, pID)) or act.stage.getType(player.x, player.y) ~= "ice"
		if doCooldown then
			player.cooldown.move = 2 * (act.stage.getType(player.x, player.y) == "sand" and 2 or 1)
		end
...
act.projectile.checkProjectileCollisions = function(info, xmod, ymod)
	xmod, ymod = xmod or 0, ymod or 0

	local struckPlayer = false
	local struckObject = false
	local cPlayer = act.player.checkPlayerAtPos(info.x + xmod, info.y + ymod) --, info.owner)
	local cObject = act.object.checkObjectAtPos(info.x + xmod, info.y + ymod) --, info.owner)
...
--						im = images.panel[stage.panels[y][x].panelType]
					...
					-- change red to blue for player two
					if stage.panels[y][x].owner == 2 then
						im = colorSwap(im, {e = "b"})
					end
					-- color panel if damaged
					dmg, flinch, invisible = act.stage.getDamage(x, y)
					if dmg > 0 and not invisible then
						im = colorSwap(im, {["7"] = "4", ["8"] = "4"})
					elseif stage.panels[y][x].panelType == "ice" then
						im = colorSwap(im, {["7"] = "8", ["8"] = "3"})
					elseif stage.panels[y][x].panelType == "metal" then
						im = colorSwap(im, {["8"] = "7"})
					elseif stage.panels[y][x].panelType == "lava" then
						im = colorSwap(im, {["7"] = "e", ["8"] = "1"})
					elseif stage.panels[y][x].panelType == "poison" then
						im = colorSwap(im, {["8"] = "a"})
					elseif stage.panels[y][x].panelType == "sand" then
						im = colorSwap(im, {["7"] = "8", ["8"] = "4"})
					end
					buffer[#buffer + 1] = {
						im,
...
local chipSelectScreen = function()
	...
	local checkIfChoosable = function(c)
		...
		for i = 1, #inQueue do
			...
			if compareChip[2] ~= chip[2] and not (chip[2] == "*" or compareChip[2] == "*") then
				isSameCode = false
			end
		end
	...
	while true do
		...
		if evt[1] == "key" then
			...
			elseif evt[2] == control.custom then
				render()
				sleep(0.1)
				os.pullEvent("key")
				sleep(0.1)
			elseif evt[2] == keys.enter then
				if #inQueue == 0 then
					cwrite("     Select some chips first!     ", 5)
					sleep(0.6)
				else
					player.chipQueue = {}
					for i = 1, #inQueue do
	...
	while true do
		...
		if isHost then
			...
			for id, object in pairs(objects) do
				...
				if dmg > 0 then
					act.object.hurtObject(id, dmg, flinching, false)
				end
				if objects[id] then
					...
					if object.frame > 1 and object.frame % object.delayedTime == 0 and object.delayedFunc then
						if not object.delayedFunc(object) then
							table.remove(objects, id)
						end
					end
				end
			...
			for id, player in pairs(players) do
				if not player.isDead then
					...
					panelType = act.stage.getType(player.x, player.y)
					if panelType == "lava" then
						if act.player.hurtPlayer(id, 50, true, false) then
							act.stage.setType(player.x, player.y, "normal")
						end
					elseif panelType == "poison" then
						act.player.hurtPlayer(id, 1, false, false)
					end
					if dmg > 0 then
						act.player.hurtPlayer(id, dmg, flinching, false)
			...
			movePlayers()
			sleep(gameDelayInit)
			game.custom = math.min(game.customMax, game.custom + game.customSpeed)
			transmit({
				gameID = gameID,
				command = "get_state",
				players = players,
		...
		if isHost then
			...
			if msg.command == "get_state" then
				os.queueEvent("ccbn_get_state", {
					players = msg.players or players,
					projectiles = msg.projectiles or projectiles,
					objects = msg.objects or objects,
					game = msg.game or game,
					stageDamage = msg.stageDamage or stage.damage,
					stagePanels = msg.stagePanels or stage.panels
				})
			end
		end
	end
...
local howToPlay = function()
	local help = {
		...
		"   ____    ____   ____   ____  ",
		"  |    |  |    | |  | | |    | ",
		"  |  "..revKeys[control.custom]:upper().." |  | <- | |  V | | -> | ",
		"  |____|  |____| |____| |____| ",
		"",
		...
		"",
		" Your health is displayed in the top",
		" left or top right corner (depending on",
		" which side you are on.) If it hits zero,",
		" you die. To help with that,",
		" dodging attacks is key! You can also",
		" use recovery chips if you're low.",
		"",
